/**
 * Sangati AI — Rule Definitions
 *
 * Declarative rules evaluated against RestaurantState.
 * Each rule has a condition (predicate) and an action (alert template).
 */
...
export interface RuleCondition {
  ...
  zoneTypes?: ZoneSnapshot extends never ? never : ('dining' | 'entry' | 'pass' | 'bar')[];
...
export const DEFAULT_RULES: Rule[] = [
  ...
  },
  {
    ...
    condition: {
      ...
      zoneTypes: ['dining'],
      custom: (_zone, state) => {
        // Also requires queue detected in entry zone
